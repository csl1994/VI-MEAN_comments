                       ...
                       const geometry_msgs::PoseStampedConstPtr &cur_pose_ptr)
{
    ...
    if (ref_it != img_pool.end() && cur_it != img_pool.end())
    {
...

        {
            ...
            // 1、设置ref_frame在base坐标系下的坐标原点
            // 2、设置ref_frame相对于base的旋转角度，这里用四元数表示
            // 3、发送变换信息
        ...
        ROS_INFO("cpu time: %fms, wall time: %fms", (clock() - t_cpu) / CLOCKS_PER_SEC * 1000, (cv::getTickCount() - t_wall) / cv::getTickFrequency());
        
        // 没仔细看，应该是滑窗倒数第一帧和第二帧的深度图？
